# api.py — FastAPI for Segmentation + Classification (robust model auto-pick + safer paths/LFS checks)
import io
import os
import base64
import cv2
...
log = logging.getLogger("forensics-api")
...
# Helpers (shared)
# =========================
def _dir_hint(models_dir: Path, allowed_exts: List[str], max_items: int = 12) -> str:
    if not models_dir.exists():
        return f"(models dir '{models_dir}' does not exist)"
    items = [p for p in models_dir.glob("*") if p.is_file()]
    if allowed_exts:
        items = [p for p in items if p.suffix.lower() in allowed_exts]
    items_sorted = sorted(items, key=lambda x: x.stat().st_mtime, reverse=True)[:max_items]
    if not items_sorted:
        return f"(no candidate weight files with extensions {allowed_exts} inside '{models_dir}')"
    return " | ".join(p.name for p in items_sorted)

def _check_lfs_pointer(p: Path):
    """
    Detect Git LFS pointer files (tiny text files). If detected, raise a clear error.
    """
    try:
        # if file is very small, check content
        if p.is_file() and p.stat().st_size < 1024 * 5:  # 5 KB heuristic
            with p.open("rb") as fh:
                head = fh.read(512)
            if b"git-lfs" in head or b"oid sha256:" in head:
                raise RuntimeError(
                    f"Detected a Git LFS pointer at: {p}\n"
                    f"Your deploy likely cloned pointer files instead of real weights.\n"
                    f"Fix: ensure 'git lfs install && git lfs pull' runs during build, or commit real .pt files."
                )
    except Exception as e:
        # Re-raise as runtime to keep behavior consistent with _ensure_or_guess
        raise RuntimeError(str(e))

def _ensure_or_guess(
    p: Optional[Union[str, Path]],
    label: str,
    models_dir: Path,
    allowed_exts: List[str]
) -> Path:
    """
    If a provided absolute/relative path doesn't exist, try the SAME filename inside MODELS_DIR.
    Emit very helpful errors with directory hints and LFS detection.
    """
    tried: List[Path] = []
    ...
    if p is not None:
        p = Path(p)
        candidates.append(p)
        # also try same filename under MODELS_DIR (handles bad absolute paths in env)
        candidates.append((models_dir / p.name).resolve())

    for c in candidates:
        if c.is_file():
            _check_lfs_pointer(c)
            return c.resolve()
        tried.append(c)

    # If we reach here, nothing resolved
    hint = _dir_hint(models_dir, allowed_exts)
    msg = textwrap.dedent(f"""
        {label} weights not found.
        Tried: {', '.join(str(x) for x in tried if x)}
        MODELS_DIR: {models_dir}
        MODELS_DIR hint: {hint}
        Tip: If you're using Git LFS, run 'git lfs install && git lfs pull' during build.
    """).strip()
...
# App & Model Load
# =========================
app = FastAPI(title="Forensics API", version="1.2")
...
def load_models():
    global SEG_MODEL, CLS_MODEL, SEG_TASK, CLS_TASK, SEG_PATH, CLS_PATH

    models_dir = settings.MODELS_DIR
    allowed = settings._allowed_exts

    # --- Resolve Segmentation ---
    seg_path = settings.SEG_WEIGHTS
    if seg_path is None:
        # Try by actual task
        seg_path = _find_weight_by_task(models_dir, allowed, expected_task="segment")
        if seg_path is None:
            # Last resort: newest file (will likely fail at runtime if not a segmenter)
            candidates = [p for p in models_dir.glob("*") if p.suffix.lower() in allowed and p.is_file()]
            if candidates:
                seg_path = max(candidates, key=lambda x: x.stat().st_mtime).resolve()
    # Validate/auto-correct common env mistakes (wrong absolute path but correct filename)
    seg_path = _ensure_or_guess(seg_path, "Segmentation", models_dir, allowed)
    seg_model = YOLO(str(seg_path))
    SEG_TASK = getattr(seg_model, "task", None)
    if SEG_TASK != "segment":
        raise RuntimeError(
            f"Expected a segmentation model, but '{Path(seg_path).name}' is task='{SEG_TASK}'. "
    ...
    if cls_path is None:
        # Try by actual task
        cls_path = _find_weight_by_task(models_dir, allowed, expected_task="classify")
        if cls_path is None:
            candidates = [p for p in models_dir.glob("*") if p.suffix.lower() in allowed and p.is_file()]
            if candidates:
                cls_path = max(candidates, key=lambda x: x.stat().st_mtime).resolve()
    cls_path = _ensure_or_guess(cls_path, "Classification", models_dir, allowed)
    ...
    if CLS_TASK != "classify":
        ...
        raise RuntimeError(
            f"Expected a classification model, but '{Path(cls_path).name}' is task='{CLS_TASK}'. "
            f"Pin the correct file via CLS_WEIGHTS or CLS_WEIGHTS_NAME."
        )

    # Assign globals
    SEG_MODEL, CLS_MODEL = seg_model, cls_model
    SEG_PATH, CLS_PATH = Path(seg_path).resolve(), Path(cls_path).resolve()
    ...
    log.info("==== Model startup summary ====")
    log.info("MODELS_DIR: %s", models_dir)
    log.info("SEGMENTATION: %s (task=%s)", SEG_PATH, SEG_TASK)
    log.info("CLASSIFICATION: %s (task=%s)", CLS_PATH, CLS_TASK)
    log.info("DEVICE: %s", DEVICE)

@app.get("/health")
def health():
    models_dir_listing = _dir_hint(settings.MODELS_DIR, settings._allowed_exts)
    return {
        "status": "ok",
        "device": str(DEVICE),
        "models_dir": str(settings.MODELS_DIR),
        "models_dir_hint": models_dir_listing,
        "seg_weights": str(SEG_PATH) if SEG_PATH else "n/a",
        "cls_weights": str(CLS_PATH) if CLS_PATH else "n/a",
        "seg_task": SEG_TASK,
        "cls_task": CLS_TASK,
        "env": {
            "MODELS_DIR": str(settings.MODELS_DIR),
            "SEG_WEIGHTS": str(settings.SEG_WEIGHTS) if settings.SEG_WEIGHTS else None,
            "CLS_WEIGHTS": str(settings.CLS_WEIGHTS) if settings.CLS_WEIGHTS else None,
            "SEG_WEIGHTS_NAME": settings.SEG_WEIGHTS_NAME,
            "CLS_WEIGHTS_NAME": settings.CLS_WEIGHTS_NAME,
        }
    }
